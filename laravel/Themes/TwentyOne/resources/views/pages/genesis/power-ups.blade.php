...
{
    /** @var array<array{repo: string, installed: bool, name: string, description: string, cover: string, includes: array<string>, pages: array<string>, models: array<string>, tables: array<string>, run_after_install: array{commands?: array<string>, factories?: array<array{model: string, count: int}>}}> */
    public array $powerups = [];

    /** @var array<array<string, bool>> */
    public array $powerupsJSON = [];

    public function mount(): void
    {
        $json = file_get_contents(public_path('/genesis/power-ups.json'));
        if ($json === false) {
            return;
        }
     ...
     * @return array{repo: string, installed: bool, name: string, description: string, cover: string, includes: array<string>, pages: array<string>, models: array<string>, tables: array<string>, run_after_install: array{commands?: array<string>, factories?: array<array{model: string, count: int}>}}
     */
    protected function fetchPowerup(string $repo, bool $installed): array
    {
    ...
    public function install(string $repo, int $index): \Illuminate\Http\RedirectResponse
    {
        ...
        $json = json_encode($this->powerupsJSON, JSON_PRETTY_PRINT);
        if ($json === false) {
            return redirect()->to('/genesis/power-ups')->withErrors(['error' => 'Failed to encode JSON']);
        }

        if (File::put($filePath, $json) === false) {
            return redirect()->to('/genesis/power-ups')->withErrors(['error' => 'Failed to save power-ups file']);
        }

        Artisan::call('powerup:install ' . $repo);

        $powerup = $this->powerups[$index] ?? null;
        if ($powerup === null) {
            return redirect()->to('/genesis/power-ups');
        }

        $run = $powerup['run_after_install'] ?? [];
